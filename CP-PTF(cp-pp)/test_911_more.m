...
load(strcat('cp-pp-911-R-', num2str(R), '-models.mat'));
res_file = strcat('cp-pp-911-R-', num2str(R),'-res.mat');
...
for k=1:n_model
    model = models{k};
    ll_all(k) = predictive_log_likelihood(model, data.all);
    curr = zeros(n_test,1);
    for j=1:n_test
        curr(j) = predictive_log_likelihood(model, data.test{j});
    end
